...
var produits = {
		...
		"eclair" : {"code":"21973", "Lait":false, "Gluten":true, "Oeuf":true, "Poisson":false, "Crustace":false, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},
		"compote de peches" : {"code":"542222", "Lait":false, "Gluten":false, "Oeuf":false, "Poisson":false, "Crustace":false, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},			
		"cake abricot" : {"code":"531293", "Lait":false, "Gluten":true, "Oeuf":false, "Poisson":false, "Crustace":false, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},			
		...
		"emmental rape" : {"code":"180", "Lait":true, "Gluten":false, "Oeuf":false, "Poisson":false, "Crustace":false, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},			
		"chair moule" : {"code":"21844", "Lait":false, "Gluten":false, "Oeuf":false, "Poisson":false, "Crustace":true, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},
		...
		"pois casse" : {"code":"4571", "Lait":false, "Gluten":false, "Oeuf":false, "Poisson":false, "Crustace":false, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},
		"ebly 10" : {"code":"604504", "Lait":false, "Gluten":true, "Oeuf":false, "Poisson":false, "Crustace":false, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},
		"ebly 5" : {"code":"604506", "Lait":false, "Gluten":true, "Oeuf":false, "Poisson":false, "Crustace":false, 
					"Mollusque":false, "Arachide":false, "F. a coque":false, "Moutarde":false, 
					"Sesame":false, "Lupin":false, "Celeri":false, "Sulfite":false, "Soja":false},	
...
//var aliments = Object.keys(prodTest);
var aliments;
var Allergenes = ["Lait", "Gluten", "Oeuf", "Poisson", "Crustace", "Mollusque", "Arachide",
				"Coque" , "Moutarde", "Sesame", "Lupin", "Celeri", "Sulfite", "Soja"];
var AllergenesCourt = ["Lait.", "Glut.", "Oeuf.", "Poiss.", "Crus.", "Moll.", "Arac.",
				"Coqu." , "Mout.", "Sésa.", "Lupi.", "Céle.", "Sulf.", "Soja."];
var quant = ["kg", "L", "g", "pc"];
//var produits = new Object();


function getReadProduit() {
	return aliments;
}
...
function readProduit() {
  ...
  fileReader.onload = function(fileLoadedEvent){
	  ...
	  //aliments = Object.keys(prodTest);
	  var al = Object.keys(prodTest);
	  aliments = al.sort();
	  console.log(aliments);
	  creeTab();
	  //document.getElementById("tableauAll").style.display = "block";
  };
	...
	$(document).ready(function() {
		...
		$( ".tags" ).autocomplete({
		  ...
		  select: function(event, ui) {
				monY = aliments.indexOf(ui.item.value);
				monX = document.activeElement.placeholder;
				appel++;
				change(monX, monY);
			  },
		});
		$( ".qua" ).autocomplete({
		  source: quantiteAppel,
		  select: function(event, ui) {},
		});
	});
	document.getElementById("valideAliment").style.display = "none";
}

// mon creeTab avec mon formulaire : fonctionne mais apres ligne 10 ecrase
/*
function creeTab() {
	//readProduit();
	...
	for (var i = 0; i < 22; i++) {	
		var line;
		line = "<form id=formuChoix> <input list=browsers onfocusout=test(this) onchange='change(this.id, this.value)' name=browser style=border:none class=produit id=prod"+ i +"> <datalist id=browsers>";
...
function creeTab() {
    ...
    for (var i = 1; i < Allergenes.length+1; i++) {
        tabl += "<td>" + i + ") " +Allergenes[i-1] + "</td>";
        //tabl += "<td><div style='writing-mode:vertical-lr'>" +Allergenes[i-1] + "</div></td>";
    }
	tabl += "</tr>"
  
	for (var i = 0; i < 20; i++) {

		tabl += "<td><select class=selecter id=" + i + " onchange='change(this.id, this.value)'>";
		tabl += "<option selected></option>";
		for (var l = 0; l < aliments.length; l++){
			tabl += "<option value=" + l + ">" + aliments[l] + "</option>";
		}
		tabl += "</select></td><td id=ref" + i + "></form></td><td><input type=text style=border:none id=quantite name=produit></td>";
		//tabl += "</select></td><td id=ref" + i + "></form></td><td><select id=quantiteSelect style=text-align:right style=margin:none style=border:none style=font-size:10 style=width:20% style=padding:none><option value='KG'>KG</option><option value='gr'> gr </option><option value='L'> L </option><option value='bt'> bt </option></select></td>";
...
//test pour convertir la focntion ok select avec le formulaire

function creeTab() {
	var tabl = "<table id=cssTableM><tr><td>Articles-Ingrédients </td><td> Code </td><td> Quantité </td><td> Unité </td>";
    /*for (var i = 1; i < Allergenes.length+1; i++) {
        tabl += "<td class='vertical-text'>" + i + ") " +Allergenes[i-1] + "</td>";
    }*/
	for (var i = 1; i < AllergenesCourt.length+1; i++) {
        tabl += "<td class='vertical-text' style='width:5em'>" + AllergenesCourt[i-1] + "</td>";
    }
	tabl += "</tr>"
  
	for (var i = 0; i < 20; i++) {
		//console.log('this.val : ' + this.value)
		//tabl += "<td><input type=text id=tags class=tagC placeholder="+i+" onchange='change("+i+", this.value)'><td id=ref" + i + "></td><td></td>";
		tabl += "<td><input type=text id=tags class=tags placeholder="+i+"><td id=ref" + i + "></td><td></td><td><input type=text class=qua style='width:50%'></td>";
...
// change ok pour ancienne méthode
/*
function change(x, y){
  for (var i=0; i< 14;i++){
		document.getElementById(x+"-"+i).innerHTML = "";
		document.getElementById("ref" + x).innerHTML = "";
	}
  //console.log(aliments[y]);
  document.getElementById("ref" + x).innerHTML = produits[aliments[y]].code;
  for (var j = 0; j < Allergenes.length; j++){
    var tab = Allergenes[j];
      if(produits[aliments[y]][tab]){
          document.getElementById(x+"-"+j).innerHTML = "X";
      }
  }
}*/

//test change pour jquery-ui
function change(x, y){
  for (var i=0; i< 14;i++){
		//x = document.getElementById("myText").placeholder;
		document.getElementById(x+"-"+i).innerHTML = "";
		document.getElementById("ref" + x).innerHTML = "";
	}
  //console.log(aliments[y]);
...
function test(t){
	for (var i=0; i< 14;i++){
		var y = t.id.substr(-1) + i; // y == [0;9][0;14] == 00 to 013 puis 10 to 113 => 90 to 913 puis retombe a 00 to 13
		//console.log(y);
...
function writeJSON() {
	//var newProduitTab = new Object();
	//sort(newProduitTab, produits, aliments);
	var filename = "savedProduits.txt";
	const aElement    = document.createElement("a");
	var texte = JSON.stringify(produits);
	//var texte = JSON.stringify(newProduitTab);
...
function retour() {
	window.location.assign("DuoCatering.html");
}
function goToAjouter() {
	window.location.assign("AjouterProduit.html");
}
function goToFiche() {
	window.location.assign("FSF.html");
}
function goToContact() {
	window.location.assign("contact.html");
}
function goToAide() {
	window.location.assign("aide.html");
}
function goToFournisseurs() {
	window.location.assign("fournisseurs.html");
}
function goToStock() {
